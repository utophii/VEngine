...
class TorusEffect : AbstractParticleEffect("torus") {
    // writes torus particle positions with variable phi density into the pooled buffer
    ...
    override fun calculateInto(buffer: ParticleBuffer, center: Location, opts: EffectOptions, time: Double) {
        ...
        // first pass: count the total number of particles so the buffer can be sized exactly
        var total = 0
        for (thetaIndex in 0 until thetaSteps) {
            for (phiIndex in 0 until phiSteps) {
                ...
                total += density.toInt().coerceAtLeast(1)
            }
        }
        buffer.acquire(total)

        // second pass: write each particle into the pooled slots
        var index = 0
        for (thetaIndex in 0 until thetaSteps) {
            ...
            for (phiIndex in 0 until phiSteps) {
                ...
                repeat(repeats) {
                    writeParticle(buffer, index, local, center, opts, time)
                    index++
                }
            }
        }
